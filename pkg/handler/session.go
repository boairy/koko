...
func checkIP(remote_addr string, curr_user string) bool {
        return true
        allow_url := "http://allow.17zjh.com/allow.conf"
        resp, err := http.Get(allow_url)

        if err != nil {
                fmt.Println("http.Get err=",err)
                logger.Infof("IP check failed: User %s, IP %s, failed to access allow ip url", curr_user, remote_addr)
                return false
        }
        ...
        result_arr := strings.Split(strings.Replace(string(bytes), ";", "allow ", -1), "allow ")
        remote_ip := net.ParseIP(remote_addr)
        if remote_ip.To4() == nil || remote_ip.To16() == nil {
                logger.Infof("IP check failed: User %s, IP %s, remote addr is invalid", curr_user, remote_addr)
                return false
        }

        for i :=0; i < len(result_arr); i++ {
                _, allow_ip, err := net.ParseCIDR(result_arr[i])
                if err != nil {
                        continue
                }

                if allow_ip.Contains(remote_ip) {
                        logger.Infof("IP check success: User %s, IP %s, user is permitted", curr_user, remote_addr)
                        return true
                }
        }
        logger.Infof("IP check success: User %s, IP %s, user is refused", curr_user, remote_addr)
        return false
}
...
func SessionHandler(sess ssh.Session) {
	...
	if ok {
                ...
                if ! checkIP(remoteAddr, sess.User()) {
                      accessMsg := "检测到当前环境[" +remoteAddr+ "]为非公司网络，请按照http://ops.17zjh.com/403.html先完成授权!\r\n"
                      utils.IgnoreErrWriteString(sess, accessMsg)
                      contactMsg := "如有疑问，请联系运维-王聪。\r\n"
                      utils.IgnoreErrWriteString(sess, contactMsg)
                      return
                }
